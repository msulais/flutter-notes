...
class _TrashPageState extends State<TrashPage> {
    ...
    void _update() async {
        ...
        List<dynamic> args = [NoteStatus.trash.name];
    ...
    void _restoreSelectedNotes() async {
        ...
        if (mounted && restore > 0) context.showSnackBar(
            Text('$restore note${restore > 1? 's': ''} restored')
        );
        _cancelEditingMode();
        _update();
    }
    ...
    void _deleteSelectedNotes() async {
        ...
        if (mounted && trash > 0) context.showSnackBar(
            Text('$trash note${trash > 1? 's': ''} deleted permanently')
        );
        _cancelEditingMode();
        _update();
    }
    ...
    void _onNoteDismissed(DismissDirection direction, Note note) async {
        if (direction == DismissDirection.startToEnd){
            ...
            await note.deleteDB();
            if (mounted) context.showSnackBar(const Text("Note deleted permanently"));
        }

        else if (direction == DismissDirection.endToStart){
            note.setStatus(NoteStatus.active);
            await note.updateDB();
            if (mounted) context.showSnackBar(const Text("Note restored"));
        }
    ...
    Widget _noteListTile(Note note){
        ...
        Widget background = Container(
            ...
            child: Row(children: [
                Icon(Icons.delete_forever_outlined, color: colorScheme.onError),
            ]),
        );
        ...
        Widget secondaryBackground = Container(
            ...
            child: Row(mainAxisAlignment: MainAxisAlignment.end, children: [
                Icon(Icons.restore_outlined, color: colorScheme.onTertiary),
            ]),
        );
    ...
    Widget _appBar(){
        ...
        return SliverAppBar(
            leading: leading,
            pinned: true,
            title: Text(_isEditing? '${_selectedNoteIds.length}' : 'Trash'),
            ...
            body = SliverFillRemaining(child: SizedBox(
                ...
                child: Column(children: [
                    ...
                    Expanded(child: Column(
                        ...
                        children: [
                            Icon(Icons.delete_outlined, size: textTheme.displayLarge?.fontSize),
                            const SizedBox(height: 16,),
                            Text('No trash', style: textTheme.titleLarge)
                        ]
                    )),
                ]),
            ));
            ...
            body = SliverList(delegate: SliverChildListDelegate([
                ...
                Card(
                    ...
                    child: ListTile(
                        ...
                        title: const Text('Note will be deleted in 7 days.'),
